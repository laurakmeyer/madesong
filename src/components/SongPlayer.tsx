  ...
  return (
    <div className="bg-white/80 rounded-2xl shadow-lg p-6 flex items-center justify-between">
      <button onClick={togglePlay}
        className="w-14 h-14 rounded-full bg-[#d97706] hover:bg-[#b45309] flex items-center justify-center text-white transition-colors shadow-md">
        {playing ? <Pause className="h-6 w-6" /> : <Play className="h-6 w-6 ml-0.5" />}
      </button>
      <div className="flex-1 mx-4">
        <div className="h-2 bg-amber-100 rounded-full">
          <div className={`h-2 bg-amber-400 rounded-full transition-all ${playing ? "animate-pulse w-1/2" : "w-0"}`} />
        </div>
      </div>
      <a href={mp3Url} download="madesong.mp3" target="_blank" rel="noopener noreferrer"
        className="flex items-center gap-1.5 text-sm text-[#d97706] hover:text-[#b45309] font-medium">
